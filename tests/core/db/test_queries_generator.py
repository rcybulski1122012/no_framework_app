...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "INSERT INTO testmodel (first, second) VALUES (%(first)s, %(second)s);",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"fields_names": ["first", "second"]},
            "UPDATE testmodel SET first=%(first)s, second=%(second)s WHERE id_=1;",
        ),
    ],
)
def test_get_update_query_when_fields_are_not_specified(
    generator_instance, kwargs, expected
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"conditions": ["first=5", "second IN (2,3,4)"]},
            "SELECT * FROM testmodel WHERE first=5 AND second IN (2,3,4);",
        ),
        (
            {
                "fields_names": ["first"],
