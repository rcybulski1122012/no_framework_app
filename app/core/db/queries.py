...
def get_insert_query(table_name, fields_names):
    formatted_fields_names = ", ".join(fields_names)
    placeholders = ", ".join([f"%({name})s" for name in fields_names])
    query = (
        f"INSERT INTO {table_name} ({formatted_fields_names}) VALUES ({placeholders});"
    )
...
def get_update_query(table_name, fields_names, conditions):
    ...
    query = f"UPDATE {table_name} SET {formatted_fields} WHERE {formatted_conditions};"
